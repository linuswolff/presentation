...
#diabetes <- subset(diabetes, !is.na(insulin) & !is.na(triceps))
#diabetes$triceps <- ifelse(is.na(diabetes$triceps), 0, diabetes$triceps)
#diabetes$insulin <- ifelse(is.na(diabetes$insulin), 0, diabetes$insulin)
count_na_per_column(diabetes)
#diabetes <- subset(diabetes, select = -insulin)
#mice <- complete(mice(subset(diabetes, select = -c(triceps, insulin)), method='rf', seed = 123))
#diabetes$glucose <- mice$glucose
#diabetes$pressure <- mice$pressure
#diabetes$mass <- mice$mass
...
# CV and tuning grid
cvControl <- trainControl(method = "cv", number = 20)
...
```
```{r}
check_collinearity(cv.lasso.model)
```
